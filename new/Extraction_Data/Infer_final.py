...
def eval_load(list_bin=None):
    ...
    before = data['bin'].tolist()
    print("2-0. input data rest data")
    all_bin = {b for b in range(0, 256)}
    print(type(all_bin))
    a = list(set(before))
    all_bin = list(all_bin)
    All_rest_length = []
    ...
    if len(all_bin) != 0:
        print("is not 256 vector", len(all_bin))
        #print(all_bin)
        length = len(data['bin'])
        #print(length)
        #print(all_bin)
        ...
        for i in all_bin:
            new_data = pd.Series([i, 0], index=['bin', 'label'])
            # print(new_data)
        ...
        for i in range(0, model_sequence_length - rest):
            new_data = pd.Series([0, 0], index=['bin', 'label'])
            # print(new_data)
            data = data.append(new_data, ignore_index=True)
        ...
        # input sequence 로 나눔, 맨 뒤 나머지 빼기
        rest = len(data) % (model_sequence_length)
        s_rest = model_sequence_length - rest
        print('3. rest data : ', rest)
        #print("s__rest", s_rest)
        # data_sub_rest = data[:len(data) - rest]
        for i in range(0, s_rest):
            new_data = pd.Series([0, 0], index=['bin', 'label'])
            # print(new_data)
            data = data.append(new_data, ignore_index=True)
        data_sub_rest = data
    ...
    print(len(set(data_sub_rest['bin'])))
    # one hot encoding
...
def anaylsis():
    ...
    for i in range(0, len(pred)):
        answer_box[i][0] = list_bin[i]
        answer_box[i][1] = hex(int(list_bin[i]))
    ...
    y_test = np.array(data0['label']).flatten()
